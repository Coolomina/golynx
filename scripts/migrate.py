...
def main():
    parser = argparse.ArgumentParser(
        description="This script migrates data from one datasource to another"
    )
    parser.add_argument(
        "--from",
        dest="from_path",
        ...
        required=True,
        help="Source database type to migrate from",
    )
    parser.add_argument(
        "--to",
        dest="to_path",
        ...
        choices=[t.value for t in DatabaseType],
        required=True,
        help="Destination database type to migrate to",
    )
